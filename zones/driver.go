...
var (
	...
	taskConfigSpec = hclspec.NewObject(map[string]*hclspec.Spec{
		...
		"Zonename":        hclspec.NewAttr("Zonename", "string", false),
...
type TaskConfig struct {
	...
	Zonename        string               `codec:"Zonename"`
...
func (d *Driver) StartTask(cfg *drivers.TaskConfig) (*drivers.TaskHandle, *drivers.DriverNetwork, error) {
	...
	z, err := d.initializeContainer(cfg, driverConfig)
	if err != nil {
		return nil, nil, fmt.Errorf("Initialize Container: %+v", err)
	}

	if err := z.WriteToFile(); err != nil {
		return nil, nil, fmt.Errorf("Cannot write file %q err=%+v", cfg.ID, err)
	}
	if err := zconfig.Register(z); err != nil {
		zconfig.Unregister(z)
		z.RemoveFile()
		return nil, nil, fmt.Errorf("Cannot Register Zone ID=%q, err=%+v zname=%s", cfg.ID, err, z.Name)
	}
	mgr, err := lifecycle.OpenZoneConfig(z.Name)
	if err != nil {
		return nil, nil, fmt.Errorf("Cannot create mgr ID=%q err=%+v+ zname=%s", cfg.ID, err, z.Name)
	}
	if err = mgr.Verify(); err != nil {
		return nil, nil, fmt.Errorf("Error Verifying zone=%s  %q, err= %+v", z.Name, cfg.ID, err)
	}
	if err = mgr.Install(nil); err != nil {
		return nil, nil, fmt.Errorf("Cannot install zone=%s ID=%q, err= %+v", z.Name, cfg.ID, err)
	}
	...
	if attrerr != nil {
		...
		zcmd := exec.Command("sh", "-c", "zlogin "+z.Name+" /usr/bin/env "+ "\"" + env + "\"" +" "+entry+" "+cmd)
		zerr := zcmd.Start()
		if zerr != nil {
			return nil, nil, fmt.Errorf("Cannot run Entrypoint  out=%q, err= %+v", zerr)
		}
		d.logger.Info("Running Entrypoint", "docker startup", hclog.Fmt("%s %s %s", env, entry, cmd))
	}
	h := &taskHandle{
		container:  zconfig.Zone{Brand: z.Brand, Zonepath: z.Zonepath},
		taskConfig: cfg,
